...
{{--                            @endif--}}
                ...
                <li class="aiz-side-nav-item">
                    <a href="{{ route('business_settings.taam_expenses') }}" class="aiz-side-nav-link">
                        <i class="las la-money-check aiz-side-nav-icon"></i>

                        <span class="aiz-side-nav-text">{{ translate('Expense taam') }}</span>
                    </a>
                </li>
                @endif
                @if(Auth::user()->user_type == 'admin' || in_array('3', json_decode(Auth::user()->staff->role->permissions)))
                ...
                @if(Auth::user()->user_type == 'admin' || in_array('5', json_decode(Auth::user()->staff->role->permissions)))
                    ...
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('transactions.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['transactions.index','transaction.uploaded_file', 'transactions.create', 'transactions.edit'])}}">
                            <i class="las la-hand-holding-usd aiz-side-nav-icon"></i>

                            <span class="aiz-side-nav-text">{{ translate('Transaction') }}</span>
                        </a>
                    </li>
                @endif
                @if(Auth::user()->user_type == 'admin' || in_array('6', json_decode(Auth::user()->staff->role->permissions)))
                    ...
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('catch_receipts.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['catch_receipts.index', 'catch_receipts.create', 'catch_receipts.edit'])}}">
                ...
                @if(Auth::user()->user_type == 'admin' || in_array('7', json_decode(Auth::user()->staff->role->permissions)))
                    ...
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('permission_exchanges.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['permission_exchanges.show','permission_exchanges.index', 'permission_exchanges.create', 'permission_exchanges.edit'])}}">
                            <i class="las la-exchange-alt  aiz-side-nav-icon"></i>

                            <span class="aiz-side-nav-text">{{ translate('Permission Exchanges') }}</span>
                    @if(Auth::user()->user_type == 'admin' )
                    <?php
                    $permissions=  \App\Models\PermissionExchange::whereApproved(0)->count();
                    ?>
                    @if($permissions > 0)
                    <span class="btn btn-icon btn-circle  btn-soft-danger"> {{$permissions}}</span>
                      @endif
                    </a>
                    </li>
                @endif
