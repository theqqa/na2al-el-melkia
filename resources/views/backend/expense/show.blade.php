...
{{--                <input type="hidden" name="lang" value="{{ $lang }}">--}}
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-9">
                        <input type="text" placeholder="{{translate('Name')}}" id="name" name="name" value="{{ $expense->getTranslation('name') }}" class="form-control" required>
                    </div>
                </div>
                <div class="form-group row">
                ...
                <div class="form-group row">
                    <label class="col-sm-3 col-from-label" for="email">{{translate('Total cost')}}</label>
                    <div class="input-group col-sm-9">
                        <input type="number" placeholder="{{translate('Total cost')}}" id="total_cost" value="{{\App\Models\PermissionExchange::whereExpenseId($expense->id)->whereStatus(1)->sum('price') + $expense->initial_balance }}" name="total_cost" class="form-control" required>
                        <div class="input-group-append">
                            <span class="input-group-text">SR</span>
                        </div>
                    </div>
                ...
                <div class="form-group mb-0 text-right">
                    <a href="{{route('expenses.index')}}"  class="btn btn-primary">{{translate('Back')}}</a>
                </div>
            </form>
        </div>
    </div>
